...
import { sliceToThreeRows } from "./PostCard";
...
describe("body slice", () => {
  test("slice postCard body", () => {
    expect(sliceToThreeRows(testData[0]).length).toBeLessThanOrEqual(36*3);
    expect(sliceToThreeRows(testData[1]).length).toBeLessThanOrEqual(36*3);
    expect(sliceToThreeRows(testData[2]).length).toBeLessThanOrEqual(36*3);
  });
});
